var bbTrivia = {
        ...
        startGame: function () {
            ...
            $('<button>')                                              // create a button
                ...
                .addClass('btn btn-correct btn-lg')                    //   style it
        ...
        play: function (current) {                                                                    // play function with current question as an argument
            if (this.qCount === this.gameQuestions.length) {                                          // if there are no more questions to ask then
                this.endGame(this.gameQuestions);                                                     // run the endGame sequence
            }
            else {                                                                                    // if there are still more questions, then
                this.qDisplay(current);                                                               //   display current question
                $('.a-option').on('click', function () {                                              //   when one of answer options is clicked
                    let guess = ($(this).data('a-val'));                                              //   store the players selection in a variable
                    current.playerChoice = guess;                                                     //   store players guess in the question object as a new key/value pair

                    // CORRECT GUESS -----------------------------------------------------------------
                    if (guess === current.correct) {                                                  // if the players guess matches the correct answer for the current question
                        ($(this).prepend('<span class="glyphicon glyphicon-ok">&nbsp;</span>'));      //   then add check mark at beginning of answer button
                        ($(this).switchClass('btn-hover', 'btn-correct', 1000));                      //   change buttons class to "correct" which shows green
                        $('.a-option').attr('disabled', 'true');                                      //   disable all buttons from further click until next question is presented
                    }
                    // WRONG GUESS -------------------------------------------------------------------
                    else if (guess !== current.correct) {
                        ($(this).prepend('<span class="glyphicon glyphicon-remove">&nbsp;</span>'));  // if the players guess does not match the correct answer
                        ($(this).switchClass('btn-hover', 'btn-wrong', 1000));                        //   then add X at the beginning of the answer selected
                        $('.a-option').attr('disabled', 'true');                                      //   disable all buttons from further click until next question is presented
                    }
                    bbTrivia.stopTimer();                                                             // stop the timer
                    $('#timer').fadeOut(250);                                                         // remove timer from display
                    // PAUSE FOR FEEDBACK ------------------------------------------------------------
                    bbTrivia.next();                                                                  // queue up next question
                })
            } // end else statement
        },  //end of play function

        // SELECT N RANDOM OBJECTS FROM A PARENT OBJECT - DAMN PROUD OF THIS ONE :) ------------------
        selectNRandom: function (object, qty) {                                          // function that takes an object array as an argument and returns a 'qty' of random items from it
            let objectArray = Object.values(object);                                     // create an array of items within the initial object passed to function
            let randomItems = [];                                                        // create an array to house selected items
            while (randomItems.length < qty) {                                           // while there still items to selected
                let item = objectArray[Math.floor(Math.random() * objectArray.length)];  // select a random item
            ...
            return randomItems;                                                          // return the selected object to the calling function
        },  // end of selectNRandom function
        ...
        qDisplay: function (current) {                                        // question display function taking a question object as an argument,
            ...
            let aOptions = [current.a1, current.a2, current.a3, current.a4];  // Array of answer Options
            $.each(aOptions, function (i, v) {                                // for each answer option
                $('<button>')                                                 // create a block level button
                    .attr('type', 'button')
                    .addClass('a-option btn btn-hover btn-lg btn-block')
                    .data('a-val', v)
                    .text(v)
                    .appendTo('#aStage');
            });
        },  //end of qDisplay function

        // NEXT QUESTION -----------------------------------------------------------------------------
        next: function () {
            setTimeout(function () {                                      // after answer option has been selected pause for 2.5 seconds
                bbTrivia.qCount++;                                        // iterate question count
                bbTrivia.play(bbTrivia.gameQuestions[bbTrivia.qCount]);   // present next question for play
            }, 2500);
        },

        // END GAME ----------------------------------------------------------------------------------
        endGame: function (qa) {
            $('#timer').empty();                // clear question stage
            $('#qStage').empty();               // clear question stage
            $('#aStage').empty();               // clear question stage
            let correctA = 0;                   // initialize a correct answer counter
            let wrongA = 0;                     // initialize a wrong answer counter
